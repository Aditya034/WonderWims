...
function Admin() {
    ...
    const handleDelete = async (tourId) => {
        try {
            const response = await fetch(`http://127.0.0.1:8443/v1/tours/${tourId}`, {
                method: "DELETE"
            });
            const result = await response.json();
            if (result.success) {
    ...
    const handleById = async (tourId) => {
        try {
            const response = await fetch(`http://127.0.0.1:8443/v1/tours/id/${tourId}`);
    ...
    const handleUpdateSubmit = async () => {
        try {
            const response = await fetch(`http://127.0.0.1:8443/v1/tours/${id}`, {
                method: "PUT",
                headers: {
    ...
    const handleCreateSubmit = async () => {
        try {
            const response = await fetch("http://127.0.0.1:8443/v1/tours/add", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json"
                },
